...
export default function PlaylistDetailScreen() {
  ...
  const [playlistData, setPlaylistData] = useState<JioPlaylist | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPlaylistData(data?.data ?? null);
      } catch {
  ...
  const coverUrl = getImageUrl(playlistData?.image);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <FlatList
        data={songs}
        keyExtractor={(item, index) => `ps-${item.id}-${index}`}
        ...
        ListHeaderComponent={
          <>
            <View style={styles.coverSection}>
              ...
              <Text
                style={[styles.playlistMeta, { color: colors.textSecondary }]}
              >
                {songs.length} songs
                {playlistData?.followerCount
                  ? ` • ${parseInt(playlistData.followerCount).toLocaleString()} followers`
                  : ""}
              </Text>
              <TouchableOpacity
